                ...
                ScrollView{
                    LazyVGrid(columns: columns, spacing: 12) {
                        ForEach(missionsViewModel.missions, id: \.id) { mission in
                            MissionComponent(taskTotal: mission.taskTotal, taskResolved: mission.taskResolved, color: mission.color, title: mission.title, description: mission.description)
                        }
                        .padding(.bottom, 8)
                    }//Vgrid
                    .padding(.horizontal)
